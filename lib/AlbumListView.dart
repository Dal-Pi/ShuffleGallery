...
class _AlbumListViewState extends State<AlbumListView> {
  ...
  void initState() {
    super.initState();
    developer.log('initState(), window.physicalSize: ${window.physicalSize}',
        name: 'SG');
  ...
  Future<void> initAsync() async {
    if (await promptPermissionSetting()) {
      ...
      for (var assetPathEntity in albumPaths) {
        developer.log(
            'name: ${assetPathEntity.name}, count: ${assetPathEntity.assetCount}',
            name: 'SG_Log');
      }
      ...
      if (albumPaths.length > 0) {
        ...
        for (var albumPath in _albumPathList) {
          List<AssetEntity> mediaPathList =
              await albumPath.getAssetListRange(start: 0, end: 1);
          if (mediaPathList.length > 0) {
            _firstMediaPathList.add(mediaPathList[0]);
          } else {
            //TODO check no_thumbnail
            developer.log('_handleScrollEvent(), there is no first media',
                name: 'SG_Log');
            _firstMediaPathList.add(AssetEntity(
                id: kNoThumbnailMediaId,
                typeInt: 1,
                height: _thumbnailWidth,
  ...
  _fetchMoreAlbumThumbnail() async {
    developer.log(
        '_fetchMoreAlbumThumbnail(), _lastPage: $_lastPage, _currentPage $_currentPage',
        name: 'SG_Log');
    _lastPage = _currentPage;
    if (await promptPermissionSetting()) {
      final int loadingItemCount = _rowCount * _rowCount * _viewHeightRatio;
      developer.log(
          '_fetchMoreAlbumThumbnail(), loadingItemCount: $loadingItemCount',
          name: 'SG_Log');
      ...
      if (begin + loadingItemCount > _albumPathList.length) {
        ...
        developer.log(
            '_fetchMoreAlbumThumbnail(), end is _albumPathList.length: $end',
            name: 'SG_Log');
      } else {
        end += loadingItemCount;
        developer.log(
            '_fetchMoreAlbumThumbnail(), end is added loadingItemCount: $end',
            name: 'SG_Log');
      }
      ...
      for (int i = begin; i < end; ++i) {
        temp.add(FutureBuilder<dynamic>(
            //TODO thumb size
            future: _firstMediaPathList[i]
                .thumbDataWithSize(_thumbnailWidth, _thumbnailWidth),
      ...
      setState(() {
        if (temp.length > 0) {
          developer.log(
              '_fetchMoreAlbumThumbnail(), loaded temp.length: ${temp.length}',
              name: 'SG_Log');
          _albumThumbnailList.addAll(temp);
          developer.log(
              '_fetchMoreAlbumThumbnail(), _albumThumbnailList.length: ${_albumThumbnailList.length}',
              name: 'SG_Log');
          _currentPage++;
        } else {
          developer.log('_fetchMoreAlbumThumbnail(), no more load item',
              name: 'SG_Log');
        }
      });
    }
          ...
          return LayoutBuilder(builder: (context, constraints) {
            return GestureDetector(
              onTap: () => Navigator.of(context).push(MaterialPageRoute(
                  builder: (context) => MediaListView(_albumPathList[index]))),
              child: Column(
                children: <Widget>[
                  ClipRRect(
